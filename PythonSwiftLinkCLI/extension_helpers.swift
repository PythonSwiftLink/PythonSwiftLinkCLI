...
extension Process {
    ...
    public func zsh_run(script: String) async throws {
        executableURL = .init(fileURLWithPath: "/bin/zsh")
        arguments = [
            "-c",
            script
        ]
...
func git(clone repos: [String], target: String) throws {
    try Process().clone(repo: repos, path: target)
}
func gitAsync(clone repos: [String], target: String) async throws {
    try Process().clone(repo: repos, path: target)
}
...
func copyItem(forced from: String, to: String) async throws {
    print("copyItem", from, to)
...
extension URL {
    ...
    var contentModification: Date {
        get {
            return (try? resourceValues(forKeys: [.contentModificationDateKey]))?.contentModificationDate ?? .init()
        }
        set {
    ...
    static func >(l: Self, r: Self) -> Bool {
        l.contentModification > r.contentModification
    }
...
extension PySwiftProject {
    static func fromString(_ string: String) -> PySwiftProject {
        var p_path = Path(string)
        let parent = p_path.absolute().parent()
        print("fromString", parent.string)
        if ROOT_PATH != p_path {
            p_path = ROOT_PATH + p_path.lastComponent
            print("p_path",p_path.string)
            print(ROOT_PATH)
        }
        let p = PySwiftProject(path: p_path)
        currentProject = p
        return p
    }
}
